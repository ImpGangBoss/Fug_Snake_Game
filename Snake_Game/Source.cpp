...
int main(int argc, char* args[])
{
	...
	else
	{
		...
		else
		{
			...
			game_over_texture.LoadFromRenderedText(renderer, font, "GAME OVER", font_color);
			score_texture.LoadFromRenderedText(renderer, font, std::to_string(score), font_color);

			background_texture.Render(renderer, 0, 0, NULL, false, false, NULL);
			snake_head_texture.Render(renderer, current_position.x, current_position.y, NULL, false, false, NULL);
			coin_texture.Render(renderer, coin_position_x, coin_position_y, false, false, 0.0);
			score_texture.Render(renderer, score_position_x, score_position_y, false, false, 0.0);
			...
			while (!quit)
			{
				...
				if (snake[0].GetPosition().y < SCREEN_BORDER
					...
					|| snake[0].GetPosition().x > SCREEN_WIDTH - snake_body_texture.GetWidth() - SCREEN_BORDER)
					game_over = true;
				...
				if (!game_over)
				{
					if (snake.size() == 1)
						...
						if ((snake[0].GetMoveDirection() == MOVE_LEFT && current_direction != MOVE_RIGHT)
							...
							|| (snake[0].GetMoveDirection() == MOVE_DOWN && current_direction != MOVE_UP))
							snake[0].SetMoveDirection(current_direction);
					...
					if (frame_counter % speed == 0)
					{
						...
						{
							add_new_segment = false;
							snake.push_back(temp_segment);
						}
						
						if (snake[0].GetPosition().x == coin_position_x && snake[0].GetPosition().y == coin_position_y)
						{
							score++;
							...
							temp_segment = snake[snake.size() - 1];
					...
					while (coin_position_x % step != 0 || coin_position_y % step != 0)
					{
